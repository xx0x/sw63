...
namespace SW63
{
    ...
    void AnimationProcessor::SetAnimation(AnimationType type)
    {
        animation_type_ = type;
        animations_[animation_type_]->Reset();
    }

    void AnimationProcessor::SetAnimation(AnimationType type, uint32_t hours, uint32_t minutes, ClockFace face, bool pm)
    {
        SetAnimation(type);
        animations_[animation_type_]->SetTime(hours, minutes, face, pm);
    }

    AnimationType AnimationProcessor::GetAnimationType()
    {
